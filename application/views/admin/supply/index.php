...
<?php $this->load->view('admin/supply/modal/index_modal') ?>
<?php $this->load->view('admin/supply/modal/checkout_modal') ?>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#view-modal_detail_barang', function(e) {
            ...
            $.ajax({
                ...
                success: function(respon) {
                    ...
                    var sub_total_supply = 0;
                    $('#sub_total_supply').html(formatNumber(sub_total_supply));
                    ...
                    $('#qty').keyup(function() {
                        sub_total_supply = 0;
                        sub_total_supply = parseInt($('#harga_supply').val()) * parseInt($(this).val());
                        $('#sub_total_supply').html(formatNumber(sub_total_supply));
                    })

                    $('#harga_supply').keyup(function() {
                        sub_total_supply = 0;
                        sub_total_supply = parseInt($('#qty').val()) * parseInt($(this).val());
                        $('#sub_total_supply').html(formatNumber(sub_total_supply));
                    })
        ...
        $('#btnSendtoCart').on('click', function(e) {
            ...
            var qty = $('#qty').val();
            var harga_supply = $('#harga_supply').val();
            $.ajax({
                'url': '<?php echo base_url() ?>Admin/send_to_cart_supply',
                'type': 'POST',
                'data': {
                    id: id,
                    qty: qty,
                    harga_supply: harga_supply,
                },
                'dataType': 'json',
                beforeSend: function() {
                    $('#btnSendtoCart').html(`
        ...
        $('#batalTransaksi').on('click', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        'url': '<?php echo base_url() ?>Admin/batal_transaksi_supply',
                        'type': 'POST',
                        'data': {
                            kd: kd
                        },
        ...
        $('#btnSendCheckout').on('click', function(e) {
            ...
            $.ajax({
                'url': '<?php echo base_url() ?>Admin/send_checkout_supply',
                'type': 'POST',
                'data': data,
                'dataType': 'json',
                beforeSend: function() {
    ...
    function loadCart() {
        ...
        $.ajax({
            'url': '<?= base_url('Admin/load_cart_supply') ?>',
            'type': 'POST',
            'dataType': 'json',
            success: function(respon) {
                if (respon.code == 200) {
    ...
    function kurang_detail(idx) {
        $.ajax({
            'url': '<?php echo base_url() ?>Admin/kurang_detail_supply',
            'type': 'POST',
            'data': {
                id: idx
            },
    ...
    function tambah_detail(idx) {
        $.ajax({
            'url': '<?php echo base_url() ?>Admin/tambah_detail_supply',
            'type': 'POST',
            'data': {
                id: idx
            },
    ...
    function batal_detail(idx) {
        $.ajax({
            'url': '<?php echo base_url() ?>Admin/batal_detail_supply',
            'type': 'POST',
            'data': {
                id: idx
            },
